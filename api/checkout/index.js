'use strict';
const { json, readBody, browserSession, upstream, publicOrigin, sameOriginRequest, safeUpstreamError } = require('../_paxinbot');
  ...
  try {
    ...
    const preferenceResponse = await fetch('https://api.mercadopago.com/checkout/preferences', {
      ...
      body:JSON.stringify({
        ...
        external_reference:String(order.externalReference),
        notification_url:`${origin}/api/webhooks/mercadopago?source_news=webhooks`,
        back_urls:{
          success:`${origin}/conta/assinatura?checkout=success&order=${encodeURIComponent(order.orderId)}`,
          pending:`${origin}/conta/assinatura?checkout=pending&order=${encodeURIComponent(order.orderId)}`,
          failure:`${origin}/conta/assinatura?checkout=failure&order=${encodeURIComponent(order.orderId)}`
        },
        auto_return:'approved',
        statement_descriptor:'PAXINBOT',
    ...
    const attached = await rpc(session.access, 'paxinbot_attach_checkout_preference', { p_order_id:order.orderId, p_preference_id:String(preference.id) });
    if (!attached.response.ok) throw new Error('attach_error');
    return json(res, 201, { ok:true, orderId:order.orderId, checkoutUrl });
  } catch {
