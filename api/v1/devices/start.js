...
module.exports = async (req, res) => {
  ...
  return json(res, 201, { ok: true, requestId, secret, userCode, expiresAt: new Date(Date.now() + 600000).toISOString(), intervalMs: 1800, verificationUrl: `${publicOrigin(req)}/activate?request=${encodeURIComponent(requestId)}&code=${encodeURIComponent(userCode)}` });
};
