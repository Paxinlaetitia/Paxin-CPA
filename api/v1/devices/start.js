...
module.exports = async (req, res) => {
  ...
  if (!await serviceRateLimit('device_start_key', identity.deviceKeyHash, 20, 600)) return json(res, 429, { ok: false, error: 'Muitas solicitações deste computador. Aguarde alguns minutos.', retryAfter: 60 }, { 'retry-after': '60' });
  ...
  if (!response.ok) return json(res, 403, { ok: false, ...safeDeviceAuthError(payload, 'Não foi possível iniciar a autorização.') });
